...
[ ] Implement 2-choice recoding datasets for `DT` (axis: `left/right` vs `down/up`) and `EmotionSwitch` (dimension: `female/male` vs `happy/sad`), including logging/excluding cross-axis/dimension responses as `lapse/outlier`.
...
[ ] Standardize hierarchical model specification: subject random effects for `v/a/t/z` (DDM), condition effects primarily on `v` (optional sensitivity: `a ~ congruency` for FLANKER), and consistent priors/sampling settings in `configs/analysis.yaml`.
