...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Apply visit3_v2_confirmed item templates to all answer_group_008 subjects, "
            "then move these subjects from item group_003 to group_004."
        )
    )
...
def main() -> int:
    ...
    group008 = _read_subject_list(interim_dir / "groups_by_answer" / "group_008_sublist.txt")
    item_group3_path = interim_dir / "groups_by_item" / "group_003_sublist.txt"
    item_group4_path = interim_dir / "groups_by_item" / "group_004_sublist.txt"
    item_group3 = _read_subject_list(item_group3_path)
    item_group4 = _read_subject_list(item_group4_path) if item_group4_path.exists() else []

    item_templates = _load_item_templates(conformed_dir / "visit3_v2_confirmed.json")
    raw_index = _index_raw_workbooks(raw_app_dir)

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_dir = raw_app_dir / f"_backup_item_replace_answer_group008_visit3v2_{ts}"
