    ...
    *,
    ...
    try:
        for sheet_name in wb.sheetnames:
            ...
            # Mirror existing conventions: ignore SST invalid last row if present.
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Replace item sequences for subjects in a given answer group that are not in item_group_001, "
            "by overwriting the Excel item column with the item_group_001 templates."
        )
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="group_001",
        help="e.g. group_001 (from data/interim/.../groups_by_answer)",
    )
...
def main() -> int:
    ...
    answer_list = _read_subject_list(interim_dir / "groups_by_answer" / f"{args.answer_group_id}_sublist.txt")
    item_group1 = _read_subject_list(interim_dir / "groups_by_item" / "group_001_sublist.txt")
    item_group1_set = set(item_group1)
    targets = [sid for sid in answer_list if sid not in item_group1_set]
    ...
    if not args.dry_run and not args.no_backup:
        ...
        backup_dir = raw_app_dir / f"_backup_item_replace_{args.answer_group_id}_{ts}"
    ...
    out_log = temp_dir / "app_v2_item_replace_answer_group_mismatch_log.json"
    out_log.write_text(
        json.dumps(
            {
                "dataset": args.dataset,
                "dry_run": bool(args.dry_run),
                "answer_group_id": args.answer_group_id,
                "backup_dir": (backup_dir.as_posix() if backup_dir else None),
                "answer_group_n": len(answer_list),
    ...
    print(f"answer_group_id={args.answer_group_id} answer_group_n={len(answer_list)}")
    print(f"item_group1_n={len(item_group1)} targets_n={len(targets)}")
