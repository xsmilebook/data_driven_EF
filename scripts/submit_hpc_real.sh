...
#SBATCH --output=outputs/logs/real_adaptive_pls/%x_%A_%a.out
#SBATCH --error=outputs/logs/real_adaptive_pls/%x_%A_%a.err
...
mkdir -p ${project_dir}/outputs/logs/real_${MODEL_TYPE}
...
python -m scripts.run_single_task \
    ...
    --log_level INFO \
    --log_file ${project_dir}/outputs/logs/real_${MODEL_TYPE}/run_${SLURM_ARRAY_TASK_ID}.log
