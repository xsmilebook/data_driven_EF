#!/usr/bin/env python3
"""
Build a per-subject BIDS-derivatives dataset containing task regressors (block HRF + event FIR)
for XCP-D task regression via custom confounds.
...
- <out_root>/sub-<label>/func/<...desc-confounds_timeseries.tsv> (task regressor matrix)
- <out_root>/confounds_config.yml (36P + task columns for XCP-D)
"""
...
def main() -> int:
    ...
    print(f"confounds_config={out_root / 'confounds_config.yml'}")
    return 0
