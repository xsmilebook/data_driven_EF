...
- Added `visit1_v1_confirmed` builder from confirmed task-source mapping (mainly visit1 + selected visit3 + Emotion2Back from visit3_v1), and applied this version to all answer_group009 raw workbooks' item columns with backup; moved answer_group009 subjects from item group_003 to group_006 and regenerated item-group manifests (`scripts/build_visit1_v1_confirmed_v2.py`, `scripts/app_v2_apply_visit1_v1_items_to_answer_group009.py`).
- Regenerated non-overwriting APP grouping outputs from current raw workbooks into `data/interim/behavioral_preprocess/groups_by_answer_regen_20260213/` and `data/interim/behavioral_preprocess/groups_by_item_regen_20260213/`, and added an item-answer matching audit under `data/interim/behavioral_preprocess/grouping_audit_regen_20260213/` (report: `docs/reports/app_grouping_regen_20260213.md`).
- Filled missing trial-12 answer (`AN`) in `EmotionStroop` for all subjects previously in `answer_003` (105/105), with backups under `data/raw/behavior_data/cibr_app_data/_backup_fill_emotionstroop_trial12_AN_20260213_153123/`; then reran and overwrote `groups_by_answer_regen_20260213`/`groups_by_item_regen_20260213`, yielding aligned 6 item groups and 6 answer groups (1:1 mapping).
- Standardized run-corrected v2 sequence directory naming by renaming `run_corrected_v2/conformed` to `run_corrected_v2/confirmed`, exported per-version 18-task two-column CSV tables under each `*_confirmed` subfolder, and generated `subject_id-referred_visit` mapping (`subject_referred_visit.csv`) for 639 subjects via `scripts/export_confirmed_visit_assets.py`; documented in `docs/reports/app_raw_data_issues_and_correction_guide.md`.
