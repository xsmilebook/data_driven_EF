...
- 若 fMRIPrep 根目录缺失或无法写入 `dataset_description.json`（xcp-d/pybids 的 BIDS 校验要求），`xcpd_task_36p_taskreg.sh` 会在工作目录创建仅含 `dataset_description.json` 的 wrapper，并将 `sub-<label>` 目录 bind 到 wrapper 下再运行。
