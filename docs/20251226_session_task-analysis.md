...
- Expanded task subject analysis to output per-task coverage, per-subject summaries, column coverage, and metadata.
- Replaced the visualization script with multi-figure summaries and saved plots under the results tree.
...
- Rewrote `src/app_data_proc/analyze_task_subjects.py` to produce detailed summaries and CSV/JSON outputs.
- Rewrote `src/app_data_proc/task_analysis_summary.py` to read new summaries and generate figures.

## Artifacts
- `src/app_data_proc/format_app_data.py`: sheet renaming with CLI options and conflict handling.
- `src/app_data_proc/analyze_task_subjects.py`: task coverage analysis and CSV/JSON outputs.
- `src/app_data_proc/task_analysis_summary.py`: visualization and summary printing.
