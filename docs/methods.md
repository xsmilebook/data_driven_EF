...
- `valid_for_acc` 与 `valid_for_rt` 均要求 `rt_min <= rt <= rt_max`。RT 缺失、非数值
  或超出范围的试次不进入准确率或 RT 指标分母。
- 不执行被试任务内 `mean + 3 SD` 高值修剪。
- 任务 QC 以 RT 筛选后 `ACC` 与随机正确率阈值比较：低于阈值时标记为
  `ok=False`；等于阈值时保留。
...
所有指标均基于 RT 筛选后的标准化长表计算。比例指标在分母为 0 时记为缺失；
`dprime` 不做有限样本校正。若命中率或虚报率分母为 0，或任一比例为 0 或 1，
则 `dprime` 记为缺失，避免向下游写入无穷值。
...
- 任务 QC 表：`subject_code`、`task`、`n_trials_raw`、`n_trials_rt_valid`、
...
- 检查关键中文列存在，RT 缺失、非数值或超出范围的试次不会进入准确率或 RT 分母。
