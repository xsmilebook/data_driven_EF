...
- `data/processed/<DATASET>/table/demo/EFNY_demo_processed.csv`: cleaned demographics.
- `data/processed/<DATASET>/table/demo/EFNY_demo_with_rsfmri.csv`: demographics merged with rs-fMRI QC (includes motion summary fields such as `meanFD` if available).
- `data/processed/<DATASET>/table/metrics/EFNY_beh_metrics.csv`: per-subject behavioral metrics wide table.
- `data/processed/<DATASET>/table/demo/EFNY_behavioral_data.csv`: merged demographics + behavioral metrics (if generated).
- `data/processed/<DATASET>/table/sublist/sublist.txt`: analysis subject list (if generated).
