...
补充：网站崩溃期的“参考被试填充错误”
- 若某一批被试的 `answer_type`（设备端字段）与日期先验一致，但 `item_type` 明显与同日/同批次多数人不一致，且符合“通过 txt 转 xlsx 补导出”的特征，则优先考虑其 item 列被错误参考被试覆盖。
- 对该类被试可执行“仅修正 item 列”的操作：用 item_group1 模板覆盖 `正式阶段刺激图片/Item名`，保留答案/按键等设备端字段不变（示例见 `docs/reports/app_v2_item_replace_answer_group2.md`）。
