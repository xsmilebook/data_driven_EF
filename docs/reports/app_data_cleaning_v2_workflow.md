...
补充：网站崩溃期的“参考被试填充错误”（仅修正 item）
- 若某批被试在答案侧（`answer_type`）与日期先验基本一致，但在 item 侧（`item_type`）明显偏离主组（item_group1），且符合“txt→xlsx 补导出”场景，则优先考虑其 `正式阶段刺激图片/Item名` 列被错误参考被试覆盖。
- 对该类被试可执行“仅修正 item 列”的操作：用 item_group1 模板覆盖 `正式阶段刺激图片/Item名`，保留答案/按键等设备端字段不变（示例与审计见 `docs/reports/app_v2_item_replace_answer_group1.md`）。
