...
- 行为时间零点：优先使用 Psychopy CSV 的首个 `MRI_Signal_s.started + MRI_Signal_s.rt` 作为扫描起点；缺失时回退到 `Begin_fix.started`，再回退到 0。
...
- 共通：
  ...
  - `response`：按键 onset = `key_resp.started + key_resp.rt`
    - 说明：`key_resp.started` 是开始收集反应的时刻，不是实际按键时刻；实际按键时间需要加上 `key_resp.rt`
- SST 额外：
  - `banana`：当 `bad` 列包含 `banana` 时，以 `Trial_image_3.started` 作为该事件 onset；非 banana 试次不生成该事件
...
3) 行为时间零点（`MRI_Signal_s.started + MRI_Signal_s.rt`）是否对应真实扫描起点；  
