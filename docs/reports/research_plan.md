...
本文件定义当前研究计划与项目层面的设计假设，并与 `ARCHITECTURE.md` 的模块化结构一致。
...
仓库组织遵循：通用逻辑可跨数据集复用，数据集差异隔离管理（参见 `ARCHITECTURE.md`）：
...
数据集专用假设（命名约定、被试 ID 规则、表结构、QC 阈值）集中记录在 `docs/workflow.md` 的 EFNY 说明部分。
...
具体输入/输出路径与 QC 逻辑为数据集特定内容，详见 `docs/workflow.md` 的 EFNY 说明部分（脚本路径为权威来源）。
...
- ABCD 专用摄取、QC 与特征定义需在新增说明文档中明确记录（开始工作时创建）。
- ABCD 的配置需与 `configs/paths.yaml` 的单数据集结构保持一致（开始工作时明确）。
...
- 所有运行产物写入 `data/` 或 `outputs/`（含 `outputs/logs/`），不纳入版本控制。
