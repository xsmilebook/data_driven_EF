...
三个任务的 CSV 中通常包含 `MRI_Signal_s.started` 与 `MRI_Signal_s.rt`。结合 Psychopy 设计代码，它们的含义分别是：

- `MRI_Signal_s.started`：开始等待 MRI 触发键的时间；
- `MRI_Signal_s.rt`：从“开始等待触发”到“收到触发键”的反应时。

因此，本项目在构建 fMRI 回归量时使用以下优先级定义行为时间零点 `t0`：

1) 首个可用的 `MRI_Signal_s.started + MRI_Signal_s.rt`（推荐口径，最接近真实扫描起点）；
...
**block/state 识别**：优先使用 `Trial_loop_list` 中包含的关键字（例如 `0back`/`2back`）判定 block 类型；若无法判定则回退为 `state_mixed`。`state_*` 仅覆盖正式刺激序列本身，不包含 block 前的任务提示图或 trial 内 fixation。
...
说明：`key_resp.started` 表示“开始收集反应”的时刻，通常与刺激 onset 同步；实际按键发生时间应为 `key_resp.started + key_resp.rt`。
...
`state_*` 仅覆盖正式刺激序列本身，不包含 block 前的任务提示图或 trial 内 fixation。
...
- `Trial_image_3.started`：覆盖层/香蕉组件出现时刻；仅当 `bad` 指向 banana/no-go 刺激时，才用于 banana 事件 onset。
- `key_resp.*`：按键与反应时。
...
- `MRI_Signal_s.started` 与 `MRI_Signal_s.rt` 是否存在且位置合理（用于 `t0 = started + rt`）。
