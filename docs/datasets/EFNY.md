...
- Canonical data roots: `data/raw/EFNY/`, `data/interim/EFNY/`, `data/processed/EFNY/`, `outputs/EFNY/`, and `outputs/EFNY/logs/`.
- Runtime artifacts: `data/` and `outputs/` (including `outputs/<DATASET>/logs/`) are not version-controlled; paths below are expected locations used by scripts.
